...
PerCapIcome_Map <-
  County_Lat_Lon %>%
  inner_join(PerCapIncome_gather,by = c("County_Name" = "County")) %>%
  filter(Year == 2017) %>%
  select(County_Name, Longitude, Latitude, Year, Per_Cap_Income)
  
```


reduce the Per_Cap_Income by divided by 1000
```{r}
for (i in 1:67){
  PerCapIcome_Map$PCI[i] <-(PerCapIcome_Map$Per_Cap_Income[i] / 1000)
}
```
...
PerCapIncomeMap <-
  ...
  addCircleMarkers(radius = ~ (PCI), color="red", popup = ~ paste("$", Per_Cap_Income))
