...
```{r}
top_10_income_county_2015 <-
  CountyTotalIncome%>%
  arrange(desc(TotIncome_2015))%>%
  head(10)

ggplot(data = top_10_income_county_2015, aes(x = County,y = TotIncome_2015 )) + 
  geom_bar(stat = 'identity', position = 'stack', width = 0.9)
  
ggplot(data = top_10_income_county_2015,aes(x = County, y = TotIncome_2015)) + 
  geom_histogram(aes(x = reorder(County, desc(TotIncome_2015))), alpha = .5) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + 
  xlab("Total income in 2015") 
```
